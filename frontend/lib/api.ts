  ...
  if (!res.ok) {
    ...
    if (res.status === 401 && token) {
      if (typeof window !== "undefined") {
        localStorage.removeItem("tg-pilot-token");
...
export const getBotNotifyConfig = (token: string) =>
  request<BotNotifyConfig>("/config/bot-notify", {}, token);

export const saveBotNotifyConfig = (
  token: string,
  config: BotNotifyConfigSaveRequest
) =>
  request<{ success: boolean; message: string }>("/config/bot-notify", {
    method: "POST",
    body: JSON.stringify(config),
  }, token);

export const deleteBotNotifyConfig = (token: string) =>
  request<{ success: boolean; message: string }>("/config/bot-notify", {
    method: "DELETE",
  }, token);

export const testBotNotify = (token: string) =>
  request<{ success: boolean; message: string }>("/config/bot-notify/test", {
    method: "POST",
  }, token);
