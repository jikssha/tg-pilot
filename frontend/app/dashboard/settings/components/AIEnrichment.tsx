        ...
        <div className="space-y-16 animate-float-up">
            <section className="space-y-8">
                <div className="flex items-center gap-4 pb-4 border-b border-white/5">
                    ...
                    <div>
                        <h2 className="text-xl font-black tracking-tight uppercase italic">Intelligence Layer</h2>
                        <p className="text-[10px] text-white/20 uppercase tracking-[0.2em] font-bold mt-0.5">{isZh ? "LLM Integration Protocol" : "LLM Integration Protocol"}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="rounded-3xl bg-white/[0.02] border border-white/5 p-8 space-y-8 shadow-inner">
                        <div className="flex items-center gap-4">
                            ...
                            <h3 className="text-sm font-bold tracking-tight">API Infrastructure</h3>
                        </div>

                        <div className="space-y-6">
                            <div className="space-y-3">
                                <div className="flex justify-between items-center ml-1">
                                    <label className="text-[10px] font-black text-white/20 uppercase tracking-widest">Secret API Access</label>
                                    {aiConfig && (
                                        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                                            <div className="w-1 h-1 rounded-full bg-emerald-500"></div>
                                            <span className="text-[9px] text-emerald-400 font-black uppercase tracking-widest">vaulted</span>
                                        </div>
                                    )}
                                </div>
                                <input
                                    ...
                                    placeholder={aiConfig ? "Enter new Key to rotate..." : "sk-..."}
                                    value={form.api_key}
                                    onChange={(e) => setForm({ ...form, api_key: e.target.value })}
                                />
                            </div>
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-white/20 uppercase tracking-widest ml-1">Base Gateway URL</label>
                                <input
                                    type="text"
                                    className="!h-12 bg-black/40 border-white/5 focus:border-purple-500/30 transition-all rounded-xl px-5 font-mono"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black text-white/20 uppercase tracking-widest ml-1">Core Intelligence Model</label>
                                <input
                                    type="text"
                                    className="!h-12 bg-black/40 border-white/5 focus:border-purple-500/30 transition-all rounded-xl px-5 font-mono"
                    ...
                    <div className="flex flex-col">
                        <div className="rounded-3xl bg-white/[0.02] border border-white/5 p-8 flex-1 bg-gradient-to-br from-white/[0.02] to-transparent shadow-inner">
                            <h3 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] pl-1 mb-8">Capabilities</h3>
                            <ul className="space-y-6">
                                <li className="flex gap-4 group">
                                    <div className="w-8 h-8 rounded-xl bg-black/20 border border-white/5 flex items-center justify-center text-[11px] font-black text-white/20 group-hover:text-purple-400 group-hover:border-purple-500/20 transition-all">01</div>
                                    <div className="space-y-1">
                                        <p className="text-xs font-bold text-white/80">Semantic Dispatching</p>
                                        <p className="text-[11px] text-white/30 leading-relaxed font-medium">Auto-distribute tasks to matching accounts via natural language understanding.</p>
                                    </div>
                                </li>
                                <li className="flex gap-4 group">
                                    <div className="w-8 h-8 rounded-xl bg-black/20 border border-white/5 flex items-center justify-center text-[11px] font-black text-white/20 group-hover:text-purple-400 group-hover:border-purple-500/20 transition-all">02</div>
                                    <div className="space-y-1">
                                        <p className="text-xs font-bold text-white/80">Dynamic Abstracting</p>
                                        <p className="text-[11px] text-white/30 leading-relaxed font-medium">Transform complex cycle logs into concise human-readable summaries.</p>
                                    </div>
                                </li>
                                <li className="flex gap-4 group">
                                    <div className="w-8 h-8 rounded-xl bg-black/20 border border-white/5 flex items-center justify-center text-[11px] font-black text-white/20 group-hover:text-purple-400 group-hover:border-purple-500/20 transition-all">03</div>
                                    <div className="space-y-1">
                                        <p className="text-xs font-bold text-white/80">Auto Diagnostics</p>
                                        <p className="text-[11px] text-white/30 leading-relaxed font-medium">Recognize error patterns and provide targeted resolution strategies.</p>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    ...
                    <div className={`p-6 rounded-2xl border animate-zoom-in flex items-center justify-between shadow-inner ${testStatus === 'success' ? 'bg-emerald-500/[0.03] border-emerald-500/10 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.05)]' : 'bg-rose-500/[0.03] border-rose-500/10 text-rose-400'}`}>
                        <div className="flex items-center gap-4">
                            ...
                            <span className="text-[11px] font-black uppercase tracking-widest">
                                {testing ? 'Synchronizing with Neural Gateway...' : testResult}
                            </span>
                        </div>
                    </div>
                )}
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-10 border-t border-white/5">
                    <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                        <button 
                            ...
                            disabled={testing}
                        >
                            <ArrowSquareOut size={18} weight="bold" className="text-purple-400" /> {isZh ? "Test Neural Link" : "Test Neural Link"}
                        </button>
                        <button 
                            className="h-12 px-8 rounded-xl border border-rose-500/10 bg-rose-500/[0.02] text-rose-400/40 text-[11px] font-black uppercase tracking-widest hover:bg-rose-500/10 hover:text-rose-400 transition-all flex items-center justify-center gap-3" 
                    ...
                    <div className="glass-panel modal-content !max-w-md !p-0 overflow-hidden animate-zoom-in border-white/5 bg-[#050505]" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-8 space-y-4 text-center">
                            <p className="text-[13px] text-white/80 leading-relaxed font-medium">
                                {isZh ? "确定要删除 AI 配置吗？所有语义调度与自动诊断功能将暂时失效。" : "Are you sure you want to delete the AI configuration? All semantic dispatching and auto-diagnostics will be temporarily disabled."}
                            </p>
                            <div className="flex items-center justify-center gap-2 text-[9px] text-purple-400/50 uppercase tracking-[0.2em] font-black italic">
                                <Warning size={12} weight="bold" />
                                Memory Bank Purge Initiated
                            </div>
                        </div>
