...
**TG-Pilot** 是一款专为高效与稳定性打造的 Telegram 多账号自动化批量签到任务管理面板。通过全新重构的 Linear 风格极简暗黑 Web 界面，你可以轻松管理多个 Telegram 账号，配置自动化签到任务，并让所有繁杂的交互操作在后台 24 小时全自动运行。`v3.8.1` 在 `v3.7.5` 的 daily-run 调度、重试补偿与今日执行概览基础上，继续修复迁移链热修复、账号状态探活、任务创建反馈和运维计数同步，并进一步统一账号工作区的侧边栏、详情区与任务卡片，让界面与运行状态都更稳定、更一致。


## ✨ 核心特性

- **🎮 多账号矩阵管理**：支持单面板统一管理无限个 Telegram 会话账号，随时查看状态。
- **💎 极简 Linear 审美 (v3.5-v3.8.1)**：全新重构的 Pure Dark 风格控制台。v3.6 优化了 **设置页面 UI**，v3.6.1 补齐了工程基线与交付护栏，v3.7 引入运维与审计视角，v3.7.5 完成 daily-run MVP，而 v3.8.1 继续统一账号工作区、任务卡片与弹窗体验，让主界面从侧栏到详情区保持同一套语言。
...
- **🛡️ 运维审计中台 (v3.7-v3.8.1)**：设置面板现已整合 **系统运维概览**、**审计事件追踪** 与 **今日执行概览**，可集中查看服务就绪检查、调度器任务、账号状态分布、签到运行摘要、daily run 执行状态与关键管理动作，并在主界面增减任务后保持更及时的同步刷新。
- **🧪 预检式迁移恢复与热修复升级 (v3.7-v3.8.1)**：会话包与全量配置包在导入前先展示 manifest、文件清单、冲突数量与可导入数量，并且旧库升级到 `v3.7.x` 时已补齐 Alembic 空表、版本识别和 schema 对齐兼容，不再要求删库重建。
- **📅 每日执行计划 MVP (v3.7.5-v3.8.1)**：新增 `daily_task_runs` 账本、分散式随机排程、daily dispatcher、失败重试与当日截止补偿，并配合今日执行概览与迁移热修复，让“每天至少执行到一次”的链路更稳定、更可观察。
